...
| Outbound-only remote portal (read-only invoices from a phone) | Three crates — `aberp-portal-core` / `-agent` / `-relay` (ADR-0115 Phase 0). The Mac polls out and opens no inbound port; the VPS front answers every un-authenticated request as a parked nginx would: no hang, no socket desynchronisation, and byte-identical across 101 enumerated request classes diffed against a live nginx — with the residual stated at the strength it holds rather than papered over: promptness everywhere, byte-parity on the enumerated classes, and outside them nginx's status class on pathological malformed input is not always reproduced. Enrolment requires Apple hardware attestation plus a confirmation typed at the Mac. Exercised end to end on loopback | A VPS and a hostname, the wildcard certificate and the two pinned Leg-B identities, and the SMTP alert credentials on the Mac — then two service units and one console enrolment | [D-20](docs/BACKLOG-designed-to-live.md#d-20) |

QC inspection results are entered by hand today; the probe sources above
are what would feed them automatically.

Two rows above are listed for their code surface rather than for a hardware
wait, and they wait on nothing external.

[D-19](docs/BACKLOG-designed-to-live.md#d-19) is a **hard gate** in the
other direction — the geometry it covers is already computed, and the entry
exists to stop a *future* pricing slice from consuming it before the known
defects are closed.

[D-20](docs/BACKLOG-designed-to-live.md#d-20) is complete code with nothing
deployed. What it needs is a VPS, a hostname, certificates and alert
credentials — an operator decision and an afternoon, not a vendor. The
portal's hostname is a deploy-time secret that deliberately never enters
this repository, which is why it cannot simply be stood up here.
