...
| Remote read-only portal (`internal.abenerp.com`) | The read routes it proxies are Live locally — `GET /health`, `GET /invoices`, `GET /invoices/:id`, `GET /invoices/:id/pdf` in `apps/aberp/src/serve.rs` — and the full design is [ADR-0113](adr/0113-internal-portal-outbound-only-remote-access.md) (outbound-only relay, WebAuthn passkeys, uniform-404 undiscoverability) | The Mac agent daemon, the VPS relay/front, the portal shell; a relay VPS, the DNS record, and a wildcard `*.abenerp.com` cert from the operator | [D-17](docs/BACKLOG-designed-to-live.md#d-17) |
