...
| Located-holes geometry, hardened enough to price off *[gate on slice C]* | ADR-0112 slice B mines `located_holes` from STEP into FeatureGraph v6 (`python/aberp-cad-extract/aberp_cad_extract/holes.py`), and the data is emitted on every extract today — but **nothing consumes it**: no quote path and no toolpath reads it, so there is zero live quoting impact | Nine geometry items — eight defects and one convention — closed **before** slice C, drilling cycle-time pricing, reads the field. **Seven are closed**: the undercut spherical cavity N4 and the dropped Ø16 bore N3 (one root cause, one fix); their mirror R2-A, a convex floor discarded and a cap taken off the part, which over-quoted a Ø12 pocket at 30012 mm in a 20 mm plate; the 118° drill-point apex, whose collapsed-isoline test compared a length against a ratio and so only held for a bore parallel to a world axis; the toroidal O-ring gland, which read a 12 mm-deep pocket as 6.5 mm and THROUGH; the **depth convention**, now ratified at both ends — a hole's depth is the drill's own travel, from the part FACE to its deepest reach, and a countersink, gland or breakout is a different operation's feature — which re-pinned seven committed rows longer; and the intersection frame anchored off the part, which lost a breakout's root at assembly-scale coordinates. **Two remain**: the zero-caps boss-overhang band and breakout holes. This is a hard gate, and all of them are pre-existing on the feature rather than regressions | [D-19](docs/BACKLOG-designed-to-live.md#d-19) |
