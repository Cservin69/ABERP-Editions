...
| Located-holes geometry, hardened enough to price off *[gate on slice C]* | ADR-0112 slice B mines `located_holes` from STEP into FeatureGraph v6 (`python/aberp-cad-extract/aberp_cad_extract/holes.py`), and the data is emitted on every extract today — but **nothing consumes it**: no quote path and no toolpath reads it, so there is zero live quoting impact | Eight open geometry items — seven defects and one undecided convention — closed **before** slice C — drilling cycle-time pricing — reads the field. Four are closed (the undercut spherical cavity N4 and the dropped Ø16 bore N3 — one root cause, one fix — their mirror R2-A, a convex floor discarded and a cap taken off the part, which over-quoted a Ø12 pocket at 30012 mm in a 20 mm plate, and the 118° drill-point apex, whose collapsed-isoline test compared a length against a ratio and so only held for a bore parallel to a world axis); four remain (the zero-caps boss-overhang band, breakout holes, the toroidal O-ring gland, and the depth convention at a relieved mouth — which round 4 showed has a second face at the far end of the bore, where a wide spherical chamber and a committed ball seat are one family and cannot both be satisfied). This is a hard gate, and all of them are pre-existing on the feature rather than regressions | [D-19](docs/BACKLOG-designed-to-live.md#d-19) |
