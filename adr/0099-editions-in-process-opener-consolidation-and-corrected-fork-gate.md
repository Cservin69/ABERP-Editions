...
|---|---|---|---|
| `ensure_consistent_with_db` (boot / pre-snapshot) | bounded, 10 s | **FATAL** — `MirrorLockTimeout`, boot refuses | it has committed nothing, so refusing loses no work, and refusing is what keeps R2's TOCTOU closed |
...
| `sync_mirror` (the 15 money-CLI tails) | **unbounded** | — | it holds no writer mutex and its commit has already landed; waiting *is* the benign outcome, and the answer it owes its caller is worth waiting for |
