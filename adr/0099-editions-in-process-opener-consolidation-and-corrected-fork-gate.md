...
> **CLOSED 2026-08-26 by [ADR-0114](0114-editions-money-cli-durability-d22.md).** The thirteen `apps/aberp` sites are Handle-routed and `durable_ack`-ed; the census grew 5 → 26. One correction to the paragraph above: `submit_invoice` was named as the target shape, and its *ordering* was indeed already right — but it never CLAIMED the flush outcome, so a failed `fsync` still printed a successful submission. So did `poll_ack` and `drain_pending_retries`. The gap had two shapes, not one. The two `aberp-snapshot::recover` sites stay open by decision (ADR-0114 §5).
...
**Round 4 correction — one row read is not enough, and "proves the whole prefix" was false as written.** A hash chain commits to its *history*, not to its own continued existence in the table. `DELETE FROM audit_ledger WHERE seq = 3` rewrites nothing — every surviving row's `entry_hash` is untouched, head included — so an interior HOLE passed the head compare and reconciled to `Unchanged`. Measured on a 5-entry ledger with seq 3 deleted: `Ok(Unchanged)`, while the mirror still held the row. That is a lost committed audit entry reported as healthy: precisely the class this ADR exists for, slipping through the check written to catch it. The proof is now head hash **AND** cardinality — still O(1). `an_interior_row_the_db_lost_is_not_reported_as_agreement` pins it.

**Round 5 correction — `COUNT(*)` is not the cardinality half.** Round 4 required the DB to hold `head.seq` *rows* over `[1..=head.seq]`. But this table carries no `UNIQUE(seq)` (S341 dropped it over duckdb#23046), so a duplicate seq is representable, and one duplicate offsets one hole exactly. `db = [1, 2, 2, 4, 5]` against `mirror = [1..=5]` has a matching head hash and `COUNT(*) = 5`, and reconciled `Ok(Unchanged)` — a lost committed entry at seq 3 reported as healthy, by the check round 4 added to catch precisely that. The proof is now `COUNT(*)` **and** `COUNT(DISTINCT seq)`, which together say what was meant all along: each of `[1..=head.seq]`, exactly once. Still one query. `a_duplicate_seq_cannot_offset_a_hole_into_agreement` pins it.
...
## R3.5 — The cross-process lock is bounded where a timeout is safe
...
Round 4 therefore bounded both takers, with **different budgets, chosen by consequence rather than by caller**.

**Round 5 — the budget was right, its placement was not.** Round 4 put the 2 s bound on `sync_mirror` *itself*. Exactly one of `sync_mirror`'s callers was the per-commit path the bound was aimed at — the one round 5 split out under the name `sync_mirror_lockstep`. The 16 that remain are all non-per-commit: fourteen are money-CLI tails in `apps/aberp` — `mark_abandoned::run`, `submit_annulment::run`, `submit_invoice`'s post-commit helper, `retry_submission`, `drain_submission_queue`, … — which call `ledger.sync_mirror(&mirror_path)?` as the *last* thing they do, after their DB commit and (on the submission paths) after NAV has already accepted the invoice. For them a budget does not bound a wedge; it converts a lock wait into a **post-commit hard error on a command that succeeded**. The re-adversarial measured it against a 5 s peer: round 4 returned `Err(MirrorLockTimeout { waited_ms: 2000 })` at 2.05 s where the pre-round-4 tree returned `Ok(1)` at 4.99 s. The constant's own doc comment — "its caller logs and continues" — was true of 1 call site in 17. (The other two of the 16 are the recovery engine, from `recover_or_refuse`'s staging paths in `aberp-snapshot/src/recover.rs`; those refuse on error and have no committed money behind them, so a bound there is harmless either way.) Counted round 6 against the tree: 16 live `.sync_mirror(` callers — 14 in `apps/aberp/src` and 2 in `crates/aberp-snapshot/src/recover.rs`; the seventeenth site, `crates/audit-ledger/src/storage/mod.rs:1161`, is inside `#[cfg(test)]` and is not a live caller.

So the bound follows the *consequence*, which means it follows the call site, not the function:

| taker | lock wait | on exhaustion | why |
|---|---|---|---|
| `ensure_consistent_with_db` (**boot**, `serve.rs` `boot_mirror_route`) | bounded, 10 s | **FATAL** — `MirrorLockTimeout` → `BootMirrorRoute::RefuseFatal`, boot refuses | it has committed nothing, so refusing loses no work, and refusing is what keeps R2's TOCTOU closed |
| `ensure_consistent_with_db` (**pre-snapshot**, `snapshot.rs` `reconcile_mirror_for`) | bounded, 10 s | **BEST-EFFORT** — `warn!`, "taking the snapshot anyway" | round 6 correction: this row was folded into the boot row and labelled FATAL, which it is not. The snapshot is a read-side copy; refusing to take one because a peer holds the mirror lock would trade a stale sidecar for no backup at all |
| `sync_mirror_lockstep` (`WriteGuard::drop`) | bounded, 2 s | **BENIGN** — `LockstepSync::SkippedLockContended`, logged, commit stands | it holds `aberp_db`'s writer mutex, so it must not park; its transaction has already committed, so it must not fail |
| `sync_mirror` (the 14 money-CLI tails + 2 recovery-engine staging calls) | **unbounded** | — | it holds no writer mutex and its commit has already landed; waiting *is* the benign outcome, and the answer it owes its caller is worth waiting for |

Splitting the entry point is what makes those two rows expressible at once: `sync_mirror` and `sync_mirror_lockstep` differ only in how they take the lock and what a contended acquire means to them, and share the body (`sync_mirror_locked`) after it.

Not zero-wait on the hot path: brief contention with the in-process reconciler is routine and a bare `try_lock` would skip spuriously.

**What the 2 s bound buys, stated honestly.** It converts an unbounded freeze into a bounded per-commit cost — it does not keep serve responsive. While a peer stays stuck, *every* commit still pays the full 2 s with the writer mutex held (measured 2.06 s per commit). That is a large improvement over 30.03 s and the peer's whole lifetime, and it is not the same thing as being fast.

**And a behind mirror is the safe direction only in one specific sense.** It is safe as ADR-0110 D3 means it: the dangerous state is a mirror running AHEAD of the DB, and a skipped sync never creates one. What it does cost is the mirror's role as the **witness**. The mirror is how a lost DB commit becomes visible: had the sync run, a later loss of those rows would surface at the next reconcile as `MirrorAheadOfDb` and be replayed back with zero row loss. With the sync skipped, DB and mirror simply agree that the rows are absent, and *that* loss is invisible — until some later, successfully-synced row is lost and the control catches that one instead. Skipping trades detection latency for liveness. It is the right trade on this path, and it is not free.
...
| `a_stuck_peer_cannot_wedge_the_per_commit_write_path` | **round 4**: a commit completes while a peer holds the mirror lock | restore **`sync_mirror_lockstep`**'s untimed `lock_exclusive()` — the commit takes the peer's full lifetime. (Round 6 correction: this row said `sync_mirror`, which after round 5's split IS the shipped untimed code — so the named mutation was a no-op and the row described a test that could not fail. The per-commit path is `sync_mirror_lockstep`.) |
...
| `a_committed_money_command_does_not_fail_because_a_peer_held_the_mirror` | **round 5**: the money-CLI tail WAITS out a peer and returns `Ok`, after its commit landed | put the budget back on `sync_mirror` — `Err(MirrorLockTimeout)` at 2.05 s, measured |
| `the_bounded_budget_is_fatal_only_where_a_timeout_is_safe` | **round 5**: one stuck peer, two takers — lockstep skips benignly at 2 s, the reconciler refuses fatally at 10 s | either half: make the lockstep fatal (`Err` at 2.04 s), or make the reconciler benign (`Ok(Unchanged)` at 10.01 s) |
| `a_duplicate_seq_cannot_offset_a_hole_into_agreement` | **round 5**: `db = [1,2,2,4,5]` vs `mirror = [1..=5]` refuses at seq 3 | drop the `COUNT(DISTINCT seq)` half — `Ok(Unchanged)` on the lost row |
...
- **`restore_into` deletes the target WAL with no evidence copy at all.** By §R3.4's own reasoning that destroys every committed row since the last checkpoint. Deliberate — a restore is a requested overwrite — but it is the one other place the §R3.4 sweep did not visit. (Round 5: this bullet had swallowed the closing sentence of the first one, which is about `recover_or_refuse`, not about `restore_into`. Returned to its owner.)
- **The 10 s lock bound is a judgement, not a measurement**, and so is the 2 s one. Every legitimate holder is orders of magnitude below them (a mirror sync = one append + one `fsync`; the reconciler = one bounded compare), and the worst plausible case — a cross-process `sync_mirror` doing a first-time backfill of a very large ledger — is still JSONL writes plus one `fsync`. If a real holder ever does exceed it, the failure is a loud, actionable, retryable boot refusal naming the path to `lsof`, not a silent or data-losing one. That asymmetry is why the bound is set where it is; if a legitimate >10 s holder is ever observed, raise the constant rather than removing the bound.
- **The fourteen money-CLI tails wait without a bound, deliberately** (round 5; count corrected round 6). A peer that holds the mirror lock forever hangs those commands forever — visibly, at the tail of a command an operator is watching, with the commit already durable. That is the failure mode chosen over reporting failure for durable work. Bounding them properly needs the D-22 inversion fixed first: while the mirror sync is still the last step *after* the money commit, there is no outcome a timeout there can honestly report. Sequenced behind D-22, not forgotten.
