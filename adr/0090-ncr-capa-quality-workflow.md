...
- `quality::grant_ncr_shipment_waiver(...)` appends one **`ncr.shipment_waiver_granted`** ledger entry carrying the actor, the NCR, the WO, the NCR's state at signing, and the stated reason — and THEN writes the row. **The ledger entry is the accountability** — not the table row, and not the state name. A state machine can be advanced by a background job; a hash-chained entry naming a person cannot.
- **Ledger BEFORE row** (amended 2026-08-26, round 8 B-2), deliberately the reverse of every sibling writer in `quality.rs`. The append runs on the shared `aberp_db::Handle` and the row on the function's residual connection: two transactions, so one can land alone. Row-first made the claim above FALSE — an append that failed left the release standing, the gate no longer listing the NCR, zero entries in the chain naming who released it, and only a 500 for the caller to notice by. The seven sibling writers in `quality.rs` record something that already HAPPENED, so a row without its entry is a reporting gap and row-first is right for them. This is the module's only RELEASE writer, so its residue must be the other one: a failed append leaves zero waiver rows and a still-blocking gate. An over-recorded intention is auditable; an unaudited release is not.
