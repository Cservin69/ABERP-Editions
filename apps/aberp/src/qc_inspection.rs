...
pub fn record_manual_inspection(
    ...
    // ADR-0099 — every audit write here (the inspection record + verdict
    // events, and the auto-NCR link) routes through the ONE shared
    // aberp_db::Handle writer instead of an independent Connection::open (the
    // opener+append was split across record_inspection/link_auto_ncr, so the
    // per-fn write-fork scanner did not flag it — but it forks the serve writer
    // just the same). Each phase acquires the write guard in its OWN scope and
    // DROPS it before the next phase / before create_ncr re-acquires it, so the
    // exclusive writer is never held nested (the S427 scope lesson, now on the
    // shared instance).
    ...
    let (recorded, plan) = {
        ...
        aberp_audit_ledger::ensure_schema(&guard)
            .map_err(|e| QcRecordError::Other(anyhow::anyhow!("ensure audit schema: {e}")))?;
